...
<body>
  ...
  ================================================== -->
	...
	<script>
		...
		// 로그인이 되어 있을때만
		// 서버측 /chat		이라는 주소로 통신을 할 수 있는 webSocket 객체 생성
		<c:if test="${!empty loginMember}">
			chattingSock = new SockJS("${contextPath}/chat");
		</c:if>
		
		// 로그인된 회원이 채팅을 입력한 후 보내기 버튼을 클릭한 경우에 채팅 내용이 서버로 전달됨
		...
		$("#send").on("click", function(){
			if(memberId == "") { //로그인이 되어있지 않은 경우
				...
				if(chat.trim().length == 0) { // 채팅이 입력되지 않은 상태로 보내기 버튼을 클릭한 경우
					...
					var obj = {}; // 비어있는 객체 선언
					obj.memberId = memberId; // obj 객체에 memberId 속성을 추가하고 값으로 memberId 변수값을 추가
					obj.chat = chat;
					
					// 작성자와 채팅 내용이 담긴 obj 객체를 JSON 형태로 변환하여 웹소켓 핸들러로 보내기
					chattingSock.send(JSON.stringify(obj));
					// JSON.stringify(obj) : 자바스크립트 객체 obj를 JSON 형태 문자열로 변환하는 자바스크립트 내장함수
					
					$("#inputChatting").val(""); // 채팅을 보낸 후 지우는 동작
				}
