...
*/
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// -> WebSocketSession은 HttpSession의 속성을 가로채서 똑같이 사용할 수 있다는 특징이 있음.
		
		logger.info("{}연결됨", session.getId());
		
		sessions.add(session);
		// 클라이언트 별로 최초 생성된 WebSocketSession을 sessions Set에 저장하여
		// 모든 접속자 정보를 저장함.
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// client -> 채팅 입력 -> Server -> 접속한 모든 사용자에게 전달
		
		// sessions에 로그인된 사용자 정보가 모두 담겨있음
		// 어떻게 알지? 
		// WebSocketSession은 servlet-context.xml에 작성된 interceptor에 의해서
		// 로그인된 사용자 정보가 담긴 HttpSession 객체를 가로채서 자기 것 처럼 사용할 수 있기 때문이다.
